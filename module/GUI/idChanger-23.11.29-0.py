...
def info(input_folder_path):
    ...
    # Sort the assigned_ids dictionary by group_id in ascending order
    sorted_ids = sorted(assigned_ids.items(), key=lambda x: x[0][0])
...
def find(input_folder, find_label, find_id):
            ...
            with open(input_file_path, 'r') as json_file:
                ...
                for shape in data['shapes']:
                    ...
                    if label == find_label and group_id == int(find_id):
                        print('label: ' + str(label) + ', ID: ' + str(group_id) +
                              ', ---filename: ' + str(filename))

    print("---------------end---------------")

def delete(input_folder, output_folder, find_label, find_id):
    assigned_ids = {}
...
def create_info_tab(root, tab_title):
    ...
    # "Process" button
    btn_process = tk.Button(tab, text="Process Folder", command=lambda: info(entry_path.get()))
    btn_process.grid(row=1, column=0, columnspan=2, pady=10)

    return tab

def create_delete_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
    ...
    tk.Label(tab, text="Delete Label:").grid(row=4, column=0, padx=10, pady=5)
    entry_find_label = tk.Entry(tab)
    entry_find_label.grid(row=4, column=1, padx=10, pady=5)

    tk.Label(tab, text="Delete ID:").grid(row=5, column=0, padx=10, pady=5)
    entry_find_id = tk.Entry(tab)
    entry_find_id.grid(row=5, column=1, padx=10, pady=5)

    # Process Button
    btn_process_folders = tk.Button(tab, text="Process Folders", command=lambda: delete(entry_input.get(), entry_output.get(), entry_find_label.get(), entry_find_id.get()))
    btn_process_folders.grid(row=6, column=0, columnspan=2, pady=10)

    return tab

def create_find_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
    ...
    btn_process_folders = tk.Button(tab, text="Process Folders",
                                    command=lambda: find(entry_path.get(),
                                                            entry_find_label.get(), entry_find_id.get()))
    btn_process_folders.grid(row=6, column=0, columnspan=2, pady=10)

    return tab

def create_unique_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
...
def create_change_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
...
def create_plus_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
...
def create_multiply_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
...
def create_devide_tab(root, tab_title):
    tab = ttk.Frame(notebook)
    notebook.add(tab, text=tab_title)
...
# Create tabs
tab1 = create_info_tab(root, "info")
tab1_1 = create_find_tab(root, "find")
tab2 = create_delete_tab(root, "delete")
tab3 = create_unique_tab(root, "unique")
tab4 = create_change_tab(root, "change")
tab5 = create_plus_tab(root, "plus")
tab6 = create_multiply_tab(root, "multiply")
tab7 = create_devide_tab(root, "devide")
