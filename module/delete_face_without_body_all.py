...
# folder_path = r'Z:\de-identification\Kakaomobility\round(20220906111759_autocardata_100)_time(1662436882_1662436931)\sensor\camera(00)\faces'

for check_folder_path in glob.glob(folder_path):
    if 'faces' in check_folder_path:
        faces_folder_path = check_folder_path + '\*'
        ...
        for faces_json_file_path in glob.glob(faces_folder_path):
            ...
            with open(faces_json_file_path, 'r') as faces_json_file:
                ...
                for shape_face in data_face['shapes']:
                    ...
                    if not os.path.exists(personID_json_file_path):
                        print("NO personID_json_file!")
                        faces_json_file.close()

                        if os.path.isfile(faces_json_file_path):
                            # os.remove(faces_json_file_path)
                            # print("---removed faces_json_file_path: ", faces_json_file_path)

                            data_face['shapes'].clear()
                    ...
                    with open(personID_json_file_path, 'r') as personID_json_file:
                        data_person = json.load(personID_json_file)
